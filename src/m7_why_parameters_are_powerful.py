...
def main():
    ...
    test_better_draw_circles(3)
    test_even_better_draw_circles(4,15,200,100,'blue')
...
def test_better_draw_circles(n):
    print()
    print('--------------------------------------------------')
...
def test_even_better_draw_circles(r_multiple,num_circles,center_x,center_y,out_color):
    print()
    print('--------------------------------------------------')
    print('Testing  even_better_draw_circles:  See graphics window')
    print('--------------------------------------------------')
    even_better_draw_circles(r_multiple, num_circles, center_x, center_y, out_color)


def even_better_draw_circles(r_multiple, num_circles, center_x, center_y, out_color):
    window = rg.RoseWindow(400, 400)
    center = rg.Point(center_x, center_y)
    for k in range(num_circles):
        circle = rg.Circle(center, k * r_multiple)
        circle.outline_color = out_color
        circle.attach_to(window)
        window.render(0.05)  # Pauses for 0.05 seconds after rendering.
    window.close_on_mouse_click()
# ----------------------------------------------------------------------
